...
conf = SparkConf().setAppName('example_app').setMaster('local[7]')
sc = SparkContext(conf=conf)

data_file = "cleanSparkData.py"
raw_data = sc.textFile(data_file)

test_data_file =  "datasets/cleanSparkDataTrain.csv"
test_raw_data = sc.textFile(test_data_file)

def new_interaction(line):
    #print (line)
    line_split = line.split(",")
    return LabeledPoint(feeling, array([float(x) for x in line_split[:-1]]))
